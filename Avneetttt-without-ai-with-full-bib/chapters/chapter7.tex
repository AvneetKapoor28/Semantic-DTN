...
A low-traffic approach works more as a sanity test, inasmuch as in conditions of low traffic, almost all communication protocols have sufficient bandwidth to operate effectively. The primary issue here is whether the use of the new router has introduced unnecessary complexity or selectivity to its operation. This seems not to be the case since the Spatio-Semantic protocol holds its own against the Epidemic protocol and excels over the Spray and Wait protocol.

The medium-traffic model characterizes the emergence of the congestion problem. Under this model, the issue of forwarding becomes important since uncontrolled copying of each copy is not possible anymore because there will be some bad consequences. The new router behaves excellently in this situation because there will still be enough chances for using the spatial and semantic information before the buffers get full. Hence, medium traffic models become very important because the protocol functions well even under normal traffic load.

The scenario with heavy traffic provides valuable insights since it reflects the challenging circumstances which trigger the thesis. In such cases, the findings distinguish between the protocols. The epidemic protocol is plagued by unlimited replication, while the Spray and Wait protocol conserves resources yet loses speed and initiative. Spatio-Semantic routing occupies the middle ground envisioned in the design chapter. It uses more transmissions than Spray and Wait, but those transmissions are directed by utility and priority rather than by blind flooding.
...
The metrics should not be interpreted in isolation. A protocol with high delivery but extreme delay may still be unsuitable for emergency communication. Similarly, the low overhead protocol may not be taking advantage of existing contacts at all. The router will do well if both measurements are considered together since it achieves high delivery while maintaining the optimal critical delay when under heavy load. Furthermore, it eliminates the worst effects of congestion caused by Epidemic routing.

Critical delivery ratio in relation to average critical delay is crucial. In high traffic conditions, Spray and Wait and Spatio-Semantic exhibit nearly identical critical delivery ratio values; however, Spatio-Semantic accomplishes a much earlier critical delivery than Spray and Wait. This distinction matters in practice because a disaster-response message that arrives much later may lose much of its operational value even if it is eventually counted as delivered. The proposed protocol therefore offers not only more reliable urgent delivery than flooding, but also more useful urgent delivery than passive waiting.
...
The findings have also reinforced the notion that it would be wise to incorporate critical-traffic awareness into congestion management as a whole. Once buffer occupancy gets threatened, priority messages should not simply be considered priority; rather, it should affect decisions regarding storage and delivery of data as well. Given the better performance of high-traffic critical delay in the proposed algorithm, this approach has both theoretical and practical merit.
