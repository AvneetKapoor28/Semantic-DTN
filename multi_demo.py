...
class MultiVisualizer:
    def __init__(self, envs, titles, width=1600, height=550):
        pygame.init()
        self.envs = envs
        self.titles = titles
        ...
        self.height = self.panel_height + 160
        ...
        pygame.display.set_caption("Live 4-Way Routing Competition Simulator")
        ...
        for idx, env in enumerate(self.envs):
            ...
            for node in env.nodes:
                ...
                if len(node.buffer) > 0:
                    glow_radius = 5 + min(len(node.buffer), 10) // 2
            ...
            critical_ratio = (
                ...
                if stats["critical_generated"] > 0 else 0.0
            )

            info_lines = [
                f"Time: {env.time}s",
                f"Delivery: {ratio*100:.1f}%",
                f"Critical: {critical_ratio*100:.1f}%",
...
def run_multi_demo():
    parser = argparse.ArgumentParser(description="DTN 4-Way Protocol Live Split-Screen")
    ...
    print("Preparing 4 identical parallel environments perfectly synced...")
